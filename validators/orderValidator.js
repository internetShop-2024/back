...
const orderValidator = async (req, res, next) => {
    try {
        if (req.body.localStorage?.length === 0) return res.status(400).json({error: "Please select goods"})
        ...
        if (req.body.cost < 400) return res.status(400).json({error: 'Minimum order amount is 400 UAH'})
